...
def transfer():
    ...
    if type(account_from) != str or type(account_to) != str or type(amount) != float:
    ...
    account_from = account_db.get_account(account_from,user_id)
    if not account_from:
        return jsonify({'message': 'Account from not found!'}), 404

    if account_from['user_id'] != user_id:
        return jsonify({'message': 'You do not have access to this account!'}), 403

    account_to = account_db.get_account(account_to,user_id)
    if not account_to:
        return jsonify({'message': 'Account to not found!'}), 404

    result = account_db.transfer_funds(account_from['account_number'], account_to['account_number'], amount)
